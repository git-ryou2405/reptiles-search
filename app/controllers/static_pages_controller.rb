class StaticPagesController < ApplicationController
  ...
  def alljapan_page
    ...
    if Reptile.all
      ...
      debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @created_at_desc.count=#{@created_at_desc.count}")  # log
      ...
      debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @new_arrivals=#{@new_arrivals.inspect}")  # log
      debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @new_arrivals.count=#{@new_arrivals.count}")  # log
    end
    
    # Topページ_searchページ共通_全国Data_検索フォーム
    if params[:searchtype] == "form"
      if params[:search].present?
        debug_log("[d] StaticPages_Ctrl: ac: alljapan_page params_search=#{params[:search]}")  # log
        ...
        if search_type_name.exists? || search_morph.exists?
          ...
          debug_log("[d] StaticPages_Ctrl: ac: alljapan_page search_type_name=#{search_type_name.inspect}")  # log
          debug_log("[d] StaticPages_Ctrl: ac: alljapan_page search_morph=#{search_morph.inspect}")  # log
        else
          if params[:search].present?
    ...
    # Topページ_全国Data_種類から探す
      ...
      debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @search_type1=#{@search_type1_list.inspect}")  # log
    ...
    # 全国Data_選択したレプタイル情報
      ...
      debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @select_reptile=#{@select_reptile.inspect}")  # log
    ...
    # 全国Data_ショップ検索
      ...
      if @search_shop_name = params[:search]
        ...
        debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @search_shop_result=#{@search_shop_result.inspect}")  # log
      ...
      # 都道府県から検索
        ...
        debug_log("[d] StaticPages_Ctrl: ac: alljapan_page @prefectures=#{@search_shop_prefectures.inspect}")  # log
